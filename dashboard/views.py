...
def todo(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                finished = request.POST["is_published"]
                if finished == "on":
                    finished = True
                else:
                    finished = False
                ...
                finished = False
            todos = Todo(
                user=request.user, title=request.POST["title"], is_published=finished
            )
            todos.save()
...
def update_todo(request, pk=None):
    ...
    if todo.is_published == True:
        todo.is_published = False
    else:
...
def profile(request):
    ...
    todos = Todo.objects.filter(is_published=False, user=request.user)
    if len(homeworks) == 0:
        homework_done = True
    else:
        homework_done = False
